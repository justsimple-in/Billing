...
import type { Client, Extra, Item } from "@/lib/types"
...
export default function CreateInvoicePage() {
  ...
  // Recalculate per-item totals whenever inputs change.
  ...
  const handleFareToggle = () => {
    ...
    setFare(nextFare)
    if (!nextFare) {
      // clearing commission + fare when disabled
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const data = await res.json()
      if (data.invoice?._id) {
        router.push(`/view/${data.invoice._id}`)
      } else {
